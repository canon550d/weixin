...
//		query.setParameterList("ids", id);
	...
	public List<Image> list(Date satrt, Date end){
		...
		*/
		 ...
		 */
		Query query = session.createQuery("select distinct i from Image i join fetch i.camera c left join fetch i.files e where i.time between :start and :end and i.state = 0 order by time");
		query.setParameter("start", satrt);
		query.setParameter("end", end);
