...
<%@taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
{
	...
	"images" : [ <c:forEach items="${list}" var="v" varStatus="vs">{
		...
		"cache" : <c:choose>
			<c:when test="${v.cache==null}">"/discovery/admin/image/preView.aspx?id=<c:out value="${v.id}" />",</c:when>
			<c:otherwise>"/discovery/admin/image/preView2.aspx?path=<c:out value="${v.cache}" />",</c:otherwise>
		</c:choose>
